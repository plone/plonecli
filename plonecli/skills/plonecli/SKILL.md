---
name: plonecli
description: Scaffold and develop Plone packages with plonecli (copier templates). Use BEFORE hand-writing any Plone artifact — content types, behaviors, views, viewlets, portlets, vocabularies, REST API services, themes, upgrade steps. Covers all plonecli commands; scaffolds GenericSetup upgrade steps after profile XML changes; adapts legacy packages (mr.bob, buildout, setup.py). Triggers on "plonecli", creating a Plone add-on or Zope project, adding a feature to one, migrating installed Plone sites.
---
